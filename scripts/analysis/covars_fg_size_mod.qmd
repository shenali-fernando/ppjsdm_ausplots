...
\
Make a neater visualisation of this table:

```{r}
#| fig-width: 10
#| fig-height: 20

#first output character strings of the formula 
form_names <- c(NULL)
for(i in 1:nrow(forms)){
vars <- forms[i, ] #to deal with empty cols 
vars <- vars[vars != "" & !is.na(vars)] #for empty and NA
f <- paste("alpha ~ 1 +", paste(vars, collapse = " + "), "+ (1 | Site_Name)")
form_names[i] <- f
}
...
perf_table
gtsave(perf_table, "glm_select_env_fgsize.docx", 
       )
```
...
```{r}
covars$t_temp <- exp(-covars$annual_mean_temp) 
range(covars$t_temp)
...
Okay based on the BIC that penalises too many terms/complexity we will throw out the group of models including 23, 5, 7, , 11, 15, 19, 22.

Based on AIC and R2 we can throw out the 'worse' models of 2, 3, 4, 6, 9, 13, 17, 21, 20.

This leaves us to decide between two groups of models that are equivalent.

1.  Models 10, 14, 18
    - R2 of 53.84% (6.4% increase from ref)

    - AIC of -123.23 with weight of 9.6%

    - BIC of -52.09 with weight of 1e6 %
2.  Models 8, 12, 16
    - R2 of 52.65% (5.21% increase from ref)

    - AIC of -124.74 with weight of 21% (difference of 1.5 units to next best model which is models 10, 14, 18)

    - BIC of -64.28 with weight of 0.0005%

```{r}
final_mods <- perf_pe2 |>
  filter(model_num %in% c(10, 14, 18, 8, 12, 16)) |> 
  select(c(model_num, Name)) |> 
  rename(Model = Name)
final_mods$Name
```

```{r}
final_forms <- forms_pe |> 
  filter(Model %in% c(final_mods$Model))

fin_mods <- left_join(final_forms, final_mods, by = "Model")

fin_mods <- fin_mods[c(1:9, 17)]

fin_mods <- fin_mods |>
  unite(formula, 2:9, sep = " + ")
```
...
ggpredict(res_pe[[14]],
  ...
  guides(colour = guide_legend(title = "Daily mean temperature"))
```
