...
```{r}
#Need a reference df quickly 

covars1 <- covars |> select(c("Site_Name", "annual_mean_temp", "P_E"))

covars1 <- covars1 |> distinct(Site_Name, .keep_all = TRUE)

covars1 <- covars1 %>% 
  mutate(georegion = case_when(Site_Name %in% c("Weeaproinah", "Turtons", "Lardner", "ANU101", "ANU363", "ANU589", "Ada Tree", "HardyCreek") ~ "VIC", 
                               Site_Name %in% c("Dawson", "Frankland", "Clare", "Giants", "Carey", "Dombakup", "Warren",  "Sutton","Collins") ~ "WA", 
                               ...
                               Site_Name %in% c("BenRidge", "Caveside", "Mackenzie", "MtField", "MtMaurice", "NorthStyx", "BondTier", "BlackRiver", "Weld", "MtField", "ZigZag", "Supersite", "Bird", "Flowerdale", "Dip") ~ "TAS")) 


sums <- covars1 |> 
  group_by(georegion) |> 
  summarise(min_MAT = min(annual_mean_temp), 
            median_MAT = median(annual_mean_temp), 
            mean_MAT = mean(annual_mean_temp), 
            max_MAT = max(annual_mean_temp), 
            min_pe = min(P_E), 
            median_pe = median(P_E), 
            mean_pe = mean(P_E), 
            max_pe = max(P_E)) |> 
  ungroup()
```
...
ggpredict(res_pe[[5]],
  ...
  guides(colour = guide_legend(title = "MAT")) + 
...
## Between-FG Linear Model

What makes sense as an useful addition to the above work is the construction of a linear model of the between functional group interactions. However these interactions are 1. small and 2. centered on zero. Which probably means they're gonna give nothing...
